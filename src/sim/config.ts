 ...
 * A56 で y 帯幅が 3.4→6.4 に拡大したことで解領域はむしろ広がる方向
 * (|Δy| の必要解が同じ帯によりゆとりを持って収まる)なので、この分離解析
 * の結論(常に解が存在する)は変わらず悪化しない。
 */
...
/**
 * アンカー高さ帯(A30 で 2.6〜6.0 に拡大 — 奥行きと分離自由度)。
 * A56 で上限を 6.0→9.0 に再拡大(「もっと高い位置にあってもいい。数は
 * 増やさなくていい、ランダムに配置」— ユーザー着想)。
 * A62 で上限を 9.0→7.5 に再調整(「まだ球体が少し角ばって見える。高さを
 * ランダムにしたことでなっているのであれば範囲を狭めてもいい」—
 * ユーザー報告)。オーケストレーターの検証(本番ビルド複数アングル)では
 * 明確なファセット再現は得られなかった(A58/A60 で実シム球・書き割り球とも
 * detail4 に統一済み)が、ユーザー体感を優先しつつ A56 の意図(「もっと
 * 高い位置」)も半分は残す妥協値として、A56 での拡大幅 3.0(6.0→9.0)の
 * ちょうど半分を戻した 7.5 を採用(6.0 + 1.5 = 7.5)。下限は不変。
 * 落下物理(FALL_G/FALL_DRAG_K)は線形抗力で終端速度 7.5 u/s 未到達のため
 * 「じわっと加速」する設計のまま。分離チェック(§2.3)は帯が広がるほど解領域が
 * 増えるため悪化しない。上限はカメラフレーミング協定(CameraRig.ts の
 * 非目標「全リング常時収容」の範囲内)。
 */
export const RING_Y_MIN = 2.6;
export const RING_Y_MAX = 7.5;
 ...
 * 外リングは帯上端寄りに離す(旧 6.0 上限当時 Δy 2.8 u)— 旧 R_MAX=1.7
 * margin 3.5 u ではフォールバック同士の水平 ≈3.0 u と合わせ
 * √(3.0²+2.8²) ≈ 4.1 > 3.5 u で必ず分離する決定的最終手段だった。A42 の
 * margin 拡大後は「フォールバック vs 既に確定した非フォールバック位置」の
 * 組み合わせで稀に及ばないケースがあり得るため、この基準位置はあくまで
 * 初期比較対象 — 実際の採否は SEPARATION_MAX_TRIES 回の試行中の最良スコアと
 * 比較して決める(A56 で上限拡大に伴い Δy は 5.8 u に増加、A62 で上限を
 * 7.5 に調整後は 4.3 u — なお旧 6.0 上限当時の 2.8 u より広く、分離には
 * 引き続き有利)。
 */
export const FALLBACK_Y_INNER = RING_Y_MIN + 0.3; // 2.9
export const FALLBACK_Y_OUTER = RING_Y_MAX - 0.3; // 7.2

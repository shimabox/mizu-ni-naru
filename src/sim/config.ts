 ...
 * 増やさなくていい、ランダムに配置」— ユーザー着想)。下限は不変。
 * 落下物理(FALL_G/FALL_DRAG_K)は線形抗力で終端速度 7.5 u/s 未到達のため
 * 「じわっと加速」する設計のまま — 上限からの落下でも着水時点でなお
 * 加速中(実測: y=9.0 の最悪ケースで落下 ≈2.8 s・着水速度 ≈5.0 u/s、
 * 中央値ケース 1.45 s・3.2 u/s の約 2 倍に伸びるが「間延び」の域(目安
 * 3〜4 s 超)には届かない)。分離チェック(§2.3)は帯が広がるほど解領域が
 * 増えるため悪化しない。上限はカメラフレーミング協定(CameraRig.ts の
 * 非目標「全リング常時収容」の範囲内)。
 */
export const RING_Y_MIN = 2.6;
export const RING_Y_MAX = 9.0;
 ...
 * 比較して決める(A56 で上限拡大に伴い Δy は 5.8 u に増加、分離にはより
 * 有利)。
 */
export const FALLBACK_Y_INNER = RING_Y_MIN + 0.3; // 2.9
export const FALLBACK_Y_OUTER = RING_Y_MAX - 0.3; // 8.7

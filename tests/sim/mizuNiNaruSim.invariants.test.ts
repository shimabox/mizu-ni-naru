...
import { BUBBLE_STATE } from '../../src/contract/WorldSpec';
...
describe('MizuNiNaruSim — プロパティ不変条件(§7.3)', () => {
  ...
  it('水位単調: Drifting/Straining 中 fill01・waterY は単調非減少、fill01 ≤ 1', () => {
    const sim = new MizuNiNaruSim();
    sim.init({ seed: 9, slotCount: 7 });
    const prevFill = new Float32Array(7).fill(-1);
    const prevWaterY = new Float32Array(7).fill(Number.NEGATIVE_INFINITY);
    const prevState = new Int32Array(7).fill(-1);
  ...
  it('counts(): SimCounts が台帳と整合する', () => {
    ...
    expect(c.bubblesActive).toBeLessThanOrEqual(7);
  ...
  it('スポナー人口: 定常で H/O が目標近傍に張り付く(視覚密度 §5.1)', () => {
    const sim = new MizuNiNaruSim();
    sim.init({ seed: 8, slotCount: 7 });
    for (let s = 0; s < 60 * 60; s++) sim.step();
    // 60 s 後、7 球中アクティブ球の平均人口が H ≈ 12、O ≈ 8 の近傍
    const c = sim.counts();

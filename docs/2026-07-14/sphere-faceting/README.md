...
detail 5の720三角形でも下端に小さな平坦部が残り、detail 6の980三角形で固定画像上の直線区間を判別できない程度まで解消した。そこで前景2レイヤーを共通factoryからdetail 6で生成する。Backdropは画面上の投影サイズが小さく角ばりを判別できないためdetail 4を維持する。
...
スクリーンショットは`prefers-reduced-motion`でCameraRigを設計上の固定時刻12秒へ止め、Chromeのvirtual timeを5秒進めて取得した。

```sh
'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome' --headless=new --hide-scrollbars --window-size=1440,727 --force-device-scale-factor=1 --force-prefers-reduced-motion=reduce --run-all-compositor-stages-before-draw --virtual-time-budget=5000 --screenshot=<output.png> 'http://127.0.0.1:5173/?seed=7&slots=24&q=0&dpr=1'
```

|変更前|変更後|
|:---:|:---:|
|![変更前](screenshots/20260714-before.png)|![変更後](screenshots/20260714-after.png)|

## レイヤー別の切り分け

時系列差を排除するため、同じrendererを使う決定論的`sim=stub`で中央手前の球を同じ範囲にcropして比較した。

### 変更前: Glass 500 / InnerWater 500 triangles

![detail 4](screenshots/diagnostics/stub-detail4.png)

青い水体の下端と透明リムの両方に直線区間が見える。

### Glassだけdetail 8

![Glass detail 8](screenshots/diagnostics/stub-glass-detail8.png)

透明な上半球のリムは滑らかになるが、青いInnerWaterの外周は残る。

### InnerWaterだけdetail 8

![InnerWater detail 8](screenshots/diagnostics/stub-inner-water-detail8.png)

青い下半球は滑らかになるが、Glassの透明リムは残る。両meshの同時対応が必要だと確認した。

### 両方detail 5 / detail 6

|detail 5・720 triangles|detail 6・980 triangles|
|:---:|:---:|
|![detail 5](screenshots/diagnostics/stub-foreground-detail5.png)|![detail 6](screenshots/diagnostics/stub-foreground-detail6.png)|

detail 5には下端の短い平坦部が残る。detail 6が視覚要件を満たす最小値と判定した。

...
部分実装・採用済み: `AtomViewAttributes`と`DropletSystem`は同一`view.step`の再uploadを省略する。120Hzで50.0%、144Hzで58.3%の要求bytes削減を確認した。正式結果は[`Atom`](performance-results/20260714-atom-step-upload.md) / [`Droplet`](performance-results/20260714-droplet-step-upload.md)を参照。camera依存のBubbleは別作業単位として残す。
